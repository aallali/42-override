...
0x0804887f <+6>:     sub    esp,0x50 // 80
// canary stuff
	0x08048882 <+9>:     mov    eax,DWORD PTR [ebp+12]
	0x08048885 <+12>:    mov    DWORD PTR [esp+28],eax
	0x08048889 <+16>:    mov    eax,gs:0x14
	0x0804888f <+22>:    mov    DWORD PTR [esp+76],eax
	0x08048893 <+26>:    xor    eax,eax
	0x08048895 <+28>:    push   eax
	0x08048896 <+29>:    xor    eax,eax
	0x08048898 <+31>:    je     0x804889d <main+36>
	0x0804889a <+33>:    add    esp,4
...
printf("-> Enter Login: ");
	...
	0x080488cf <+86>:    mov    eax,ds:0x804a060 // <stdin@@GLIBC_2.0>
	0x080488d4 <+91>:    mov    DWORD PTR [esp+0x8],eax
	0x080488d8 <+95>:    mov    DWORD PTR [esp+0x4],0x20 // 32
	0x080488e0 <+103>:   lea    eax,[loginBuffer]
...
printf("-> Enter Serial: ");
	...
	0x08048922 <+169>:   lea    edx,[serialBuffer]
	0x08048926 <+173>:   mov    DWORD PTR [esp+4],edx
	0x0804892a <+177>:   mov    DWORD PTR [esp],eax
	0x0804892d <+180>:   call   0x80485e0 <__isoc99_scanf@plt>
scanf("%u", serialBuffer);
	0x08048932 <+185>:   mov    eax,DWORD PTR [serialBuffer]
	0x08048936 <+189>:   mov    DWORD PTR [esp+4],eax
	0x0804893a <+193>:   lea    eax,[loginBuffer]
...
0x08048962 <+233>:   mov    eax,0
0x08048967 <+238>:   jmp    0x804896e <main+245>

0x08048969 <+240>:   mov    eax,1
0x0804896e <+245>:   mov    edx,DWORD PTR [esp+76]
...
0x08048981 <+264>:   ret   
```
### 0x08048748 : auth(login, serial) : disassembly
- notebook: (to convert `hex` to `dec` and assign variable names for better reading)
```c
...
0x0804874b <+3>:     sub    esp,40

	0x0804874e <+6>:     mov    DWORD PTR [esp+4],0x8048a63 // "\n"
	0x08048756 <+14>:    mov    eax,DWORD PTR [login]
	0x08048759 <+17>:    mov    DWORD PTR [esp],eax
	0x0804875c <+20>:    call   0x8048520 <strcspn@plt>
strcspn(login, "\n");
	0x08048761 <+25>:    add    eax,DWORD PTR [login]
	0x08048764 <+28>:    mov    BYTE PTR [eax],0
	0x08048767 <+31>:    mov    DWORD PTR [esp+4],32
	0x0804876f <+39>:    mov    eax,DWORD PTR [login]
	0x08048772 <+42>:    mov    DWORD PTR [esp],eax
	0x08048775 <+45>:    call   0x80485d0 <strnlen@plt>
	0x0804877a <+50>:    mov    DWORD PTR [loginLength],eax
loginLength = strnlen(login, 32); // return length of login excluding null bytes \0 but max length will be 32

0x0804877d <+53>:    push   eax
0x0804877e <+54>:    xor    eax,eax // loginLength ^ loginLength
0x08048780 <+56>:    je     0x8048785 <auth+61>
0x08048782 <+58>:    add    esp,4
0x08048785 <+61>:    pop    eax
0x08048786 <+62>:    cmp    DWORD PTR [loginLength],5
...
if (loginLength > 0) {
	...
	0x08048796 <+78>:    mov    DWORD PTR [esp+12],0
	0x0804879e <+86>:    mov    DWORD PTR [esp+8],1
	0x080487a6 <+94>:    mov    DWORD PTR [esp+4],0
	0x080487ae <+102>:   mov    DWORD PTR [esp],0
	0x080487b5 <+109>:   call   0x80485f0 <ptrace@plt>

	0x080487ba <+114>:   cmp    eax,0xffffffff // -1
	0x080487bd <+117>:   jne    0x80487ed <auth+165>

	0x080487bf <+119>:   mov    DWORD PTR [esp],0x8048a68 // "\033[32m.", '-' <repeats 27 times>, "."
	...
	0x080487d7 <+143>:   mov    DWORD PTR [esp],0x8048ab0 // "\033[32m'", '-' <repeats 27 times>, "'"
...
0x080487ed <+165>:   mov    eax,DWORD PTR [login]
0x080487f0 <+168>:   add    eax,3
0x080487f3 <+171>:   movzx  eax,BYTE PTR [eax]
0x080487f6 <+174>:   movsx  eax,al
0x080487f9 <+177>:   xor    eax,0x1337 // 4919
0x080487fe <+182>:   add    eax,0x5eeded // 6221293
0x08048803 <+187>:   mov    DWORD PTR [key],eax
key = (login[2] ^ 4919) + 6221293
0x08048806 <+190>:   mov    DWORD PTR [i],0
i = 0

0x0804880d <+197>:   jmp    0x804885b <auth+275>

0x0804880f <+199>:   mov    eax,DWORD PTR [i]
0x08048812 <+202>:   add    eax,DWORD PTR [ebp+0x8]
0x08048815 <+205>:   movzx  eax,BYTE PTR [eax]
0x08048818 <+208>:   cmp    al,31
0x0804881a <+210>:   jg     0x8048823 <auth+219>
if (login[i] <= 31) {
		0x0804881c <+212>:   mov    eax,1
		0x08048821 <+217>:   jmp    0x8048877 <auth+303>
	return (1)
}

hashAlgo
	0x08048823 <+219>:   mov    eax,DWORD PTR [i]
	0x08048826 <+222>:   add    eax,DWORD PTR [ebp+8]
	0x08048829 <+225>:   movzx  eax,BYTE PTR [eax]
	0x0804882c <+228>:   movsx  eax,al
	0x0804882f <+231>:   mov    ecx,eax
	0x08048831 <+233>:   xor    ecx,DWORD PTR [key]
...
0x08048863 <+283>:   mov    eax,DWORD PTR [login]
0x08048866 <+286>:   cmp    eax,DWORD PTR [key]
